...
6. 同一个路由桶内，客户端同格式的 native 候选先于跨格式 adapter 候选；adapter 仍保留为 fallback，但不会再仅因探测延迟更低而压过健康 native 候选。转换路径带 `ADAPTER_LATENCY_PENALTY_MS`，默认 250ms，用于 adapter 候选之间排序；路由日志会记录 `upstream_kind`、`format_adapter`、`message_content_types`、`tool_types` 和 `tool_key_sets`，便于判断是否发生了跨格式转换以及工具协议来源。
7. Chat stream + tools 的路由判断按单个 health item 执行：同一请求可同时拥有规范化 Chat 候选、普通 Responses 候选和 Codex-compatible Responses 候选；最终按路由组、native/adapter 层级、优先级、权重和延迟选择，并在失败时继续尝试下一个候选。
